    ...
    def issuebook(self):
        class test(main):
            ...
            def issue(self):
                ...
                self.fmi = Canvas(
                    self.f, bg="#ffe8ec", width=900, height=390, bd=0, relief="flat"
                )
                self.fmi.place(x=0, y=0)

                self.fc = Frame(
                    self.fmi, bg="#ffe8ec", width=338, height=230, bd=4, relief="flat"
                )
                self.fc.place(x=70, y=20)

                self.ffbll = Frame(
                    self.fc, bg="#00203f", bd=2, relief="flat", width=210, height=40
                )
                ...
                self.em2 = Entry(
                    self.fc, width=30, bd=5, relief="ridge", font=("Arial", 8, "bold")
                )
            ...
            def check(self):
                ...
                if self.var != None:
                    self.fmii = Canvas(
                        self.f, bg="#ffe8ec", width=338, height=90, bd=0, relief="flat"
                    )
                    ...
                    self.ff = Frame(
                        self.fr, bg="#adefd1", bd=2, relief="flat", width=140, height=40
                    )
                    ...
                    self.bt1 = Button(
                        ...
                        relief="flat",
                        command=self.data,
                        activeforeground="#adefd1",
                        activebackground="#00203f",
                    )
                    self.bt1.place(x=15, y=160)
                else:
                    messagebox.showwarning(
                        "Warning", "This student is not registered !"
                    )
            ...
            def issued(self):
                ...
                if self.n <= 3:
                    ...
                    book.execute(
                        "SELECT * FROM Students WHERE ERP='" + self.erpid1 + "'"
                    )
                    self.issuevar = book.fetchone()
                    self.sum = self.issuevar[11] + 1
                    book.execute(
                        "UPDATE Students SET NoBook='"
                        + str(self.sum)
                        + "' WHERE ERP='"
                        + self.b
                        + "' "
    ...
    def returnbook(self):
        class retu(main):
            def __init__(self):
                ...
                self.frame = Frame(
                    root, bd=0, relief="flat", bg="#ffe8ec", width=900, height=390
                )
                self.frame.place(x=0, y=110)
                self.f1 = Frame(
                    self.frame, bg="#ffe8ec", width=500, height=200, bd=5, relief="flat"
                )
                self.f1.place(x=200, y=15)
                self.ed = Frame(
                    self.f1, bg="#581845", bd=0, relief="flat", width=130, height=35
                )
                self.ed.place(x=170, y=0)
                self.lac = Label(
                    self.ed,
            ...
            def retbook(self):
                ...
                if self.data != None:
                    if int(self.data[11]) >= 1:
                        ...
                        cursor = dbstudents.cursor()
                        cursor.execute(
                            "SELECT * FROM Students WHERE ERP='" + self.entry + "'"
                        )
                        dbstudents.commit()
                        self.var = cursor.fetchone()
                        ...
                        messagebox.showwarning(
                            "No Books Found",
                            "This student does not have any book issued!",
                        )
                        self.entry4.delete(0, END)

                else:
                    messagebox.showerror(
                        "Invalid ERP ID", "This student doesn't exist!"
                    )
                    self.entry4.delete(0, END)
